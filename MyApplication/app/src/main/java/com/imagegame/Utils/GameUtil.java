package com.imagegame.Utils;

import com.imagegame.PuzzleMainActivity;
...
public class GameUtil {
    ...
    public static boolean isMoveable(int position) {
        int type = PuzzleMainActivity.TYPE;
        // 获取空格Item
    ...
    public static void getPuzzleGenerator() {
        ...
        for (int i = 0; i < mItemBeans.size(); i++) {
            index = (int) (Math.random() *
                    PuzzleMainActivity.TYPE * PuzzleMainActivity.TYPE);
        ...
        for (ItemBean tempBean : GameUtil.mItemBeans) {
            if (tempBean.getBitmapId() != 0 &&
                ...
                continue;
                    ...
                    tempBean.getItemId() == PuzzleMainActivity.TYPE * PuzzleMainActivity.TYPE) {
                continue;
            } else {
                return false;
            }
    ...
    public static boolean canSolve(List<Integer> data) {
        ...
        if (data.size() % 2 == 1) {
            ...
            // 从底往上数,空格位于奇数行
            if (((blankId - 1) / PuzzleMainActivity.TYPE) % 2 == 1) {
                return getInversions(data) % 2 == 0;
            } else {
                // 从底往上数,空位位于偶数行
